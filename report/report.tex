...
\begin{center}
	\begin{tikzpicture}[node distance=2cm]
		...
		\node[io, below of=start](io){input};
		\node[process, below of=io](pro1){Process 1};		
		\node[decision, below of=pro1,yshift=-0.5cm](dec1){Decission 1};	
		\node[startstop, below of=dec1,yshift=-0.5cm](stop){Stop};		
		
		%arrows			
		\draw[flowArrow](start)--(io);
		\draw[flowArrow](io)--(pro1);		
		\draw[flowArrow](pro1)--(dec1);				
		\draw[flowArrow](dec1)--(stop);						
	\end{tikzpicture}
\end{center}
...
\begin{center}
	\begin{tikzpicture}
		...
		\node[block,below of=signalGenerator](1stClassifier){1stClassifier};		
		...
		\path[line](signalGenerator)--(1stClassifier);		
